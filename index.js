...
app.get("/kick-players.mp4", (req, res) => {
  res.sendFile(path.join(__dirname, "public/2024-04-14 12-42-01.mp4"));
});
...
function downloadYouTubeVideo(socket, videoUrl, directoryPath) {
  ...
  if (!videoUrl.includes("www.youtube.com") && !videoUrl.includes("youtu.be")) {
    socket.emit(
      "notify",
      "Heads up!",
...
io.on("connection", (socket) => {
  ...
  socket.on("joinOrCreateRoom", (roomName) => {
    const existingChannel = channels.find((ch) => ch.channelName === roomName);
    let usernames = [];
  ...
  socket.on("custom", (msg) => {
    connectedSockets.forEach((sock) => {
      if (sock.channelID == socket.channelID) {
        ...
        sock.emit("vc", { username: socket.username, vcData: msg.data.vcData });
      }
    });
  });
  ...
  socket.on("admin message", ({ username, permissions, adminPassword }) => {
    ...
    if (!username) {
      ...
      connectedSockets.forEach((sock) => {
        if (sock.username == username) {
          userIP = sock.handshake.headers["x-forwarded-for"];
        }
      });
    }
    ...
    fs.readFile("./admins.json", (err, data) => {
      ...
      if (
        adminPassword ===
        "snowlychat:super-secret-admin-password-secure@" + currentVersion
      ) {
      ...
      fs.writeFile("./admins.json", JSON.stringify(admins, null, 2), (err) => {
        if (err) {
      ...
      if (sock.channelID == socket.channelID) {
        ...
        fs.readFile("./admins.json", (err, data) => {
          ...
          if (msg.split(" ")[0] == ">notify" && socket.isAdmin) {
            ...
            let description = parsedDescription.split(" color: ")[0];
